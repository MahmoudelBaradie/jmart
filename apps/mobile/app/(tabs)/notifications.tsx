...
export default function NotificationsScreen() {
  ...
  const markRead = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['mobile-notifications'] }); qc.invalidateQueries({ queryKey: ['mobile-unread-count'] }); },
  });
      ...
      <View style={styles.header}>
        ...
        {unread > 0 && (
          <TouchableOpacity onPress={() => markAll.mutate()}>
            <Text style={styles.markAllBtn}>تعيين الكل كمقروء</Text>
          </TouchableOpacity>
        )}
      </View>
