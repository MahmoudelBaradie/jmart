...
export default function FarmersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['farmers', page, search, kycStatus],
    queryFn: () =>
      farmersApi.list({ page, limit, search: search || undefined, kycStatus: kycStatus || undefined })
        .then((r) => r.data),
  });
  ...
  const kycMutation = useMutation({
    mutationFn: ({ status }: { status: string }) =>
      farmersApi.kycReview(selected!.id, status, kycNotes),
    onSuccess: () => {
                ...
                <tbody className="divide-y divide-gray-100">
                  {farmers.map((f: {
                    ...
                    <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 font-medium text-gray-900">{f.businessName}</td>
                      <td className="px-4 py-3 text-gray-600">{f.contactPersonName}</td>
                      <td className="px-4 py-3 text-gray-500">{f.contactPhone}</td>
                      <td className="px-4 py-3"><StatusBadge status={f.kycStatus} /></td>
                      ...
                      <td className="px-4 py-3">
                        {f.kycStatus === 'PENDING' && (
                          <Button
                            variant="secondary"
                            size="sm"
                            onClick={() => setSelected({ id: f.id, businessName: f.businessName })}
                          >
                            KYC Review
                          </Button>
                        )}
